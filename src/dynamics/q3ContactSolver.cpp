	...
	for (i32 i = 0; i < m_contactCount; ++i) {
		...
		for (i32 j = 0; j < cs->contactCount; ++j) {
			...
			for (i32 i = 0; i < 2; ++i) {
				...
				tm[i] += q3Dot(raCt, cs->iA * raCt)
						+ q3Dot(rbCt, cs->iB * rbCt);
				c->tangentMass[i] = q3Invert(tm[i]);
			}

			// Precalculate bias factor
			c->bias = -Q3_BAUMGARTE * (r32(1.0) / dt)
					* q3Min(r32(0.0), c->penetration + Q3_PENETRATION_SLOP);
...
// Warm start contact
			...
			r32 dv = q3Dot(vB + q3Cross(wB, c->rb) - vA - q3Cross(wA, c->ra),
					cs->normal);
...
//				float bias = -Q3_BAUMGARTE * (60.0f)
//						* q3Min(r32(0.0), c->penetration + Q3_PENETRATION_SLOP);

				float bias = c->penetration;
						//-0.2f * (60.0f)
						//* q3Min(r32(0.0), c->penetration + 0.05f);

				cout << "penetration: " << c->penetration << endl;

				// Factor in positional bias to calculate impulse scalar j
				r32 lambda = c->normalMass * (-vn - bias);

				//	cout << "bias: " << c->bias << endl;
